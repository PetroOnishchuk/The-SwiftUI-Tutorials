...
class MyAppFileManager {
    ...
    func takeContentsOfDirectory(addPath: String) -> [String] {
        let emptyFile = MyFile(fileName: "", fileExtension: "", textForFile: "", image: nil, typeOfFile: "")
        var arrayOfItems = [String]()
        
        guard let mainURL = takeMainDirectoryURL(addPath: addPath, myFile: emptyFile) else {
            return [String]()
        }
        ...
        do {
            ...
            arrayOfItems = try manager.contentsOfDirectory(atPath: path)
        } catch let error {
            print("Error Description \(error.localizedDescription)")
        }
    ...
    func takeAttributes(addPath: String, name: String) -> FileAttributeType? {
        let newFile = MyFile(fileName: name, fileExtension: "", textForFile: "", image: nil, typeOfFile: "")
        
        guard let mainURL = takeMainDirectoryURL(addPath: addPath, myFile: newFile) else {
            return nil
        }
